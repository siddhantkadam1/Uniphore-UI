...
export default function CommonTable() {
  ...
  const { setDialogFlag, dialogFlag } = useAdminDialog();
  ...
  const { searchTeams } = useContext(SearchContext)
  ...
  }, [toggle.callTeamsListApi]);


  useEffect(() => {
    setLoading(true);
    axios.post(`${env.BASE_URL}:${env.PORT}/udops/team/list_search_team/ `, { teamname_substring: searchTeams })
      .then(response => {
        setRows(response.data.data)
        setLoading(false);
  ...
  const openPanel = (data, editFlag) => {
    if (editFlag == true) {
      setToggle({ ...toggle, uacToggle: !toggle.uacToggle, uacObj: data, isEditUac: true });
    } else {
      setToggle({ ...toggle, uacToggle: !toggle.uacToggle, uacObj: data, isEditUac: false });
    }
  }

  const deleteTeam = (data) => {
    // is user is not an admin,don't allow him to delete the team 
    if(!isAdmin(data?.admin_user_names)) {
      setMsg("Access Denied:You must be an admin of this team to delete this team")
      setSuccess('error');
      setOpen(true);
      return;
    }
    ...
    setFlag(true);
    axios.post(`${env.BASE_URL}:${env.PORT}/udops/team/delete_team/ `, payload)
      .then(res => {
        setOpen(true);
                      ...
                      {(rowsPerPage > 0
                        ...
                        <TableRow key={index}>
                          ...
                          <TableCell scope="row" align="left" style={{ color: "#012148" }}   >
                            {
                              row.admin_user_names && row.admin_user_names.length > 0
                                ?
                                <span style={{ color: '#358AEF', cursor: 'pointer' }} onClick={() => { setDialogFlag(true); setDialogData(row.admin_user_names) }}>
                                  View admins
                                </span>
                                :
                                <span>--</span>
                          ...
                          <TableCell scope="row" align="left" onMouseOut={() => setIdx(rows.length + 7)} className='dots' onMouseLeave={() => setShowMenu(false)} >
                            <div className='menu-container' onMouseOver={() => setIdx(index)}>
                              ...
                              <Menu index={index} idx={idx} row={row} teamsTable={true} openPanel={openPanel} deleteTeam={deleteTeam} />
                            </div>
                          </TableCell>
                        </TableRow>
                      ))}
