...
export default function MyCorpusTable() {
    ...
    const { searchMycorpus, setSearchMycorpus } = useContext(SearchContext)


    useEffect(() => {
        setLoading(true);
        axios.post(`${env.BASE_URL}:${env.PORT}/udops/corpus/search/`, { corpus_name: searchMycorpus })
            .then(response => {
    ...
    }, [searchMycorpus, toggle.callListApi]);
    ...
    const callPush = (data) => {
        setCorpusState({ ...corpusState, push: true, skipStep: true, corpusObj: data })
    }

    const callPull = (data) => {
        setCorpusState({ ...corpusState, pull: true, corpusObj: data })
    }
                            ...
                            rows.length > 0 ? <div className='table-container'>
                                <TableContainer>
                                    <Table sx={{ minWidth: 500 }} aria-label="custom pagination table">
                                        ...
                                        {/* <TableHead>
                                            <TableRow>
                                                <StyledTableCell align="left">Corpus Name</StyledTableCell>
                                            ...
                                            {(rowsPerPage > 0
                                                ...
                                                <TableRow key={index}>
                                                    <TableCell scope="row" align="left" style={{ color: "#012148", padding: 12 }}>
                                                        <span onClick={() => openPanel(row)} className='table-cell no-underline'>
                                                            {row.corpus_name ? row.corpus_name : '--'}
                                                        </span>
                                                    </TableCell>
                                                    <TableCell scope="row" align="left" style={{ color: "#012148" }}>
                                                        {row.lastupdated_ts ? row.lastupdated_ts : '--'}
                                                    </TableCell>
